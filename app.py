...
def load_color_dataset():
    ...
    # 1. If CSV exists in repo root:
    if os.path.exists("color_names.csv"):
        df = pd.read_csv("color_names.csv")
        return build_tree(df)

    # 2. If ZIP fallback exists (your Streamlit upload)
    if os.path.exists("color set.zip"):
        try:
            with zipfile.ZipFile("color set.zip", 'r') as z:
    ...
    # 3. Last fallback → user upload
    uploaded = st.file_uploader("Upload color_names.csv", type=["csv"])
    if uploaded:
...
def build_tree(df):
    df["rgb"] = df.apply(lambda x: (x["Red (8 bit)"],
                                   x["Green (8 bit)"],
                                   x["Blue (8 bit)"]), axis=1)
